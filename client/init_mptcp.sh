...
#ip rule add from 10.10.1.3 table 1
#ip rule add from 10.10.2.3 table 2
for i in 1 2; do
	ip rule add from ${ips[$i]} table $i
done;

# Configure the two different routing tables
#ip route add 10.10.1.0/24 dev enp0s8 scope link table 1
#ip route add default via 10.10.1.2 dev enp0s8 table 1
#ip route add 10.10.2.0/24 dev enp0s9 scope link table 2
#ip route add default via 10.10.2.2 dev enp0s9 table 2
for i in 1 2; do
